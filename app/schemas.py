...
# Define the schema for the individual audio file metadata
class AudioFile(BaseModel):
    ...
    def validate_audio_length(cls, encoded_audio: str):
        ...
        # Ensure the audio length is greater than 0
        if len(audio_data) == 0:
            raise ValueError("Audio file is empty or has no valid data")
        
        # You can also set a minimum length if needed (e.g., at least 1 second of audio)
        min_length_samples = 4000  # 1 second at 4000 Hz sample rate
...
class ProcessAudioRequest(BaseModel):
    ...
    class Config:
        # This will ensure that datetime is parsed correctly from ISO-8601 strings
        json_encoders = {
            datetime: lambda v: v.isoformat()
        }

# Define the schema for the response payload
class ProcessAudioResponse(BaseModel):
    ...
    class Config:
        # This will allow you to define a custom JSON format for the response
        json_encoders = {
            datetime: lambda v: v.isoformat()
        }